...
export default function App(){
 ...
 const evalScore=materialEval(game);
 ...
 function press(sq:Square){setHint(null);if(thinking||game.isGameOver()||(mode==='computer'&&game.turn()==='b'))return;const p=game.get(sq);if(!selected){if(p&&p.color===game.turn())setSelected(sq);return}if(selected===sq){setSelected(null);return}try{const n=copyGame(game);n.move({from:selected,to:sq,promotion:'q'});setLast({from:selected,to:sq});setSelected(null);setGame(n);botReply(n)}catch{if(p&&p.color===game.turn())setSelected(sq);else setSelected(null)}}
 function fresh(){setGame(new Chess());setSelected(null);setLast(null);setHint(null);setThinking(false)}
 function undo(){if(thinking)return;const n=copyGame(game);if(mode==='computer'){n.undo();if(n.turn()==='b')n.undo()}else n.undo();const h=n.history({verbose:true});const lm=h[h.length-1];setGame(n);setSelected(null);setHint(null);setLast(lm?{from:lm.from,to:lm.to}:null)}
 function showHint(){const moves=game.moves({verbose:true});if(!moves.length)return;const ranked=[...moves].sort((a,b)=>(b.captured?pieceValue[b.captured]:0)-(a.captured?pieceValue[a.captured]:0));setHint({from:ranked[0].from,to:ranked[0].to});setSelected(ranked[0].from)}
 ...
 return <div className="app">
  ...
  <main><section className="game">
   <div className="player-row opponent"><div className="avatar">KZ</div><div><b>{mode==='computer'?'KnightZero Bot':'Player 2'}</b><small>{thinking?'Thinking…':'Ready'}</small></div></div>
   <div className="board-shell">
    <div className="eval-bar" aria-label={`Position evaluation ${evalScore>0?'+':''}${evalScore}`}><div className="eval-white" style={{height:`${whiteShare}%`}}/><span>{evalScore===0?'0.0':`${evalScore>0?'+':''}${evalScore.toFixed(1)}`}</span></div>
    <div className="board" role="grid">{squares.map((sq,index)=>{const p=game.get(sq);const f=files.indexOf(sq[0]);const r=Number(sq[1]);const dark=(f+r)%2===1;const target=legal.has(sq);const recent=!!last&&(last.from===sq||last.to===sq);const hinted=!!hint&&(hint.from===sq||hint.to===sq);const row=Math.floor(index/8),col=index%8;const cellStyle:CSSProperties={left:`${col*12.5}%`,top:`${row*12.5}%`,width:'12.5%',height:'12.5%'};return <button key={sq} style={cellStyle} onClick={()=>press(sq)} className={`square ${dark?'dark':'light'} ${selected===sq?'active':''} ${recent?'recent':''} ${hinted?'hinted':''}`} aria-label={sq}>{p&&<span className={`piece ${p.color}`}>{glyph[p.color+p.type]}</span>}{target&&<i className={p?'capture':'dot'}/>} {col===0&&<span className="rank-label">{sq[1]}</span>}{row===7&&<span className="file-label">{sq[0]}</span>}</button>})}</div>
   </div>
   <div className="player-row you-row"><div className="avatar you">YOU</div><div><b>You</b><small>{status}</small></div></div>
   <div className="move-strip"><button aria-label="Previous move">‹</button><div>{latest?<><span>{Math.ceil(history.length/2)}.</span><strong>{latest.san}</strong></>:<span>Game ready</span>}</div><button aria-label="Next move">›</button></div>
  </section></main>
